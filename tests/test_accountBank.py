...
def test_create_account():
    ...
    response = client.post("/accounts", json=invalid_account_data)
    assert response.status_code == 404
    assert response.json() == {"detail": "User with id <662ee1c28c59e5cee83d1992> no found"}
...
def test_update_balance():
    ...
    response = client.patch(f"/accounts/{id_account}?amount={new_amount_positive}")
    assert response.status_code == 200
    ...
    response = client.patch(f"/accounts/{id_account}?amount={new_amount_negative}")
    assert response.status_code == 200
    ...
    response = client.patch(f"/accounts/662ee1c28c59e5cee83d1992?amount={new_amount_negative}")
    assert response.status_code == 404

    #update the balance of an account with an invalid format
    response = client.patch(f"/accounts/id_invalid_format?amount={new_amount_negative}")
...
def test_replace_balance():
    ...
    response = client.put(f"/accounts/662ee1c28c59e5cee83d1992?balance={new_balance}")
    assert response.status_code == 500
